                     ...
                     <ul>
                        <li class="d-none d-lg-inline-block d-lg-inline-block">
                           <div class="it-header-top-contact d-flex align-items-center">      
                              ...
                              <a class="border-line" href="tel:{{ preg_replace('/[^0-9]/', '', $siteSetting->hotline) }}">{{ $siteSetting->hotline }}</a>
                           </div>
                        </li>
                        <li>
                           <div class="it-header-top-contact d-flex align-items-center">
                              ...
                              <a class="border-line" href="mailto:{{ $siteSetting->email }}">{{ $siteSetting->email }}</a>
                           </div>
                        </li>
                     </ul>
                  </div>
                        ...
                        @auth
                           <div class="d-inline-flex align-items-center gap-2">
                              ...
                              @if(Auth::user()->isAdmin())
                                 <a href="{{ route('backend.admin.dashboard') }}" class="text-white text-decoration-none">
                                    <i class="far fa-user-cog"></i> Quản trị
                                 </a>
                                 <span class="text-white-50">|</span>
                              @endif
                              <a href="{{ route('frontend.logout.get') }}" class="text-white text-decoration-none" 
                                 onclick="event.preventDefault(); document.getElementById('header-logout-form').submit();">
                                 <i class="far fa-sign-out-alt"></i> Đăng xuất
                              </a>
                              <form id="header-logout-form" action="{{ route('frontend.logout') }}" method="POST" class="d-none">
                                 @csrf
                              </form>
                           </div>
                        @else
                           <a href="{{ route('frontend.login') }}">Đăng nhập</a>
                           <span>/</span>
                           <a href="{{ route('frontend.register') }}">Đăng ký</a>
                        @endauth
                     </div>
...
<header class="it-header-height it-header-sticky-top">
            ...
            <div class="row align-items-center">
               <div class="col-xxl-2 col-xl-2 col-lg-4 col-md-5 col-6">
                  <div class="it-header-logo">
                     <a href="{{ route('frontend.home') }}"><img src="{{ $siteSetting->logo_url }}" alt="" style="max-width: 160px; max-height: 55px; width: auto;"></a>
                  </div>
               </div>
                        ...
                        <ul>
                           @if (isset($headerMenus) && $headerMenus->isNotEmpty())
                              @foreach ($headerMenus as $menu)
                                 @include('frontend.partials.menu-item', ['menu' => $menu])
                              @endforeach
                           @else
   ...
   <style>
      /* Thêm chút style cho toast để nhìn cao cấp hơn */
