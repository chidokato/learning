   ...
   <div class="it-course-item mb-35">
      <div class="it-course-thumb z-index-1 border-radius-20" style="position: relative; overflow: hidden;">
         <a href="{{ $course->frontend_url }}" class="d-block w-100">
            <img class="w-100" style="height: 230px; object-fit: cover; object-position: center; display: block;" src="{{ $course->image ? asset($course->image) : asset('assets/img/course/course-1-' . ((($index ?? 0) % 9) + 1) . '.jpg') }}" alt="{{ $course->title }}">
         </a>
         @if ($course->category)
            <span class="course-category">{{ $course->category->name }}</span>
         @else
            <span class="course-category">Khóa học</span>
         @endif
